    ...
    <div v-else>
      ...
      <div class="tab-content">
        ...
        <div v-if="activeTab === 'graph'" class="tab-panel">
          ...
          <div v-if="currentView === 'visualize'" class="graph-visualization">
            <div class="graph-controls">
              <div class="control-section">
                <h3>Graph Controls</h3>
                <div class="control-buttons">
                  <button class="btn primary" @click="loadVisualization">
                    Load Graph
                  </button>
                  <button class="btn secondary" @click="refreshGraph">
                    Refresh
                  </button>
          ...
          <div v-if="currentView === 'explore'" class="graph-explorer">
            ...
            <div class="search-section">
              ...
              <div class="search-controls">
                ...
                <button class="btn primary search-btn" @click="searchGraph">
                  Search
                </button>
              </div>
            </div>
            ...
            <div class="query-section">
              ...
              <div class="query-controls">
                <textarea 
                  ...
                  rows="3"
                ></textarea>
                <button class="btn primary query-btn" @click="executeCypherQuery">
                  Execute Query
                </button>
                <button class="btn secondary clear-btn" @click="clearResults">
                  Clear
                </button>
              </div>
            </div>
                ...
                <div v-for="(item, index) in searchResults" :key="index" class="result-card">
                  <div class="result-header">
                    ...
                    <div class="result-actions">
                      <button class="btn-icon edit" @click="editNode(item)" title="Edit">
                        ✏️
                      </button>
                      <button class="btn-icon delete" @click="deleteNode(item.id)" title="Delete">
                        🗑️
                      </button>
                    </div>
                  </div>
            ...
            <div class="stats-section">
              ...
              <div class="stats-grid">
                ...
                <div class="stat-item">
                  ...
                  <span class="stat-value">{{ Object.keys(graphStats.nodeTypes).join(', ') }}</span>
                </div>
                <div class="stat-item">
                  <span class="stat-label">Last Updated:</span>
                  <span class="stat-value">{{ new Date(graphStats.lastUpdated).toLocaleString() }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="currentView === 'edit'" class="graph-editor">
            ...
            <div class="create-section">
              ...
              <form @submit.prevent="createNewNode" class="node-form">
                <div class="form-row">
                ...
                <div class="form-row">
                  ...
                  <input v-model="newNode.label" type="text" required class="form-input" placeholder="Node name" />
                </div>
                <div class="form-row">
                ...
                <div class="form-actions">
                  ...
                  <button type="button" class="btn secondary" @click="resetCreateForm">Reset</button>
                </div>
              </form>
            </div>
            ...
            <div class="create-section">
              <h3>Create Edge</h3>
              <form @submit.prevent="createNewEdge" class="edge-form">
                <div class="form-row">
                  <label>From Node ID:</label>
                  <input v-model="newEdge.from_node" type="text" required class="form-input" placeholder="Source node ID" />
                </div>
                <div class="form-row">
                  <label>To Node ID:</label>
                  <input v-model="newEdge.to_node" type="text" required class="form-input" placeholder="Target node ID" />
                </div>
                <div class="form-row">
                ...
                <div class="form-actions">
                  ...
                  <button type="button" class="btn secondary" @click="resetEdgeForm">Reset</button>
                </div>
              </form>
            </div>
          </div>
          ...
          <div v-if="showEditModal" class="modal-overlay" @click="closeEditModal">
            <div class="modal-content" @click.stop>
              <h3>Edit Node</h3>
              <form @submit.prevent="updateNode" class="edit-form">
                <div class="form-row">
                  <label>Node Type:</label>
                  <input v-model="editingNode.type" type="text" required class="form-input" />
                </div>
                <div class="form-row">
                  <label>Label/Name:</label>
                  <input v-model="editingNode.label" type="text" required class="form-input" />
                </div>
                <div class="form-row">
                  <label>Properties (JSON):</label>
                  <textarea 
...
const newEdge = ref({ from_node: "", to_node: "", relationship_type: "DROPS", properties: {} });
...
function renderGraph(data: any) {
  ...
  // Create nodes
  const nodeElements = data.nodes.map((node: any, index: number) => {
  ...
  // Create edges
  const edgeElements = data.edges.map((edge: any) => {
...
async function updateNode() {
  try {
    ...
    const response = await fetch(`/api/graph/nodes/${editingNode.value.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(nodeData)
    });
...
function resetEdgeForm() {
  newEdge.value = { from_node: "", to_node: "", relationship_type: "DROPS", properties: {} };
