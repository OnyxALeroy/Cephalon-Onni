<template>
  <div class="creative-mode">
      ...
      <form @submit.prevent="handleBuildSubmit" class="form">
        ...
        <div class="form-actions">
          <button type="submit" class="btn-primary" :disabled="loading || !isFormValid">
            ...
            <span v-else>{{ editingBuild?.id ? 'Update' : 'Save' }} Build</span>
          </button>
          
          <button type="button" class="btn-secondary" @click="backToList" :disabled="loading">
            Cancel
          </button>
        </div>
      </form>
    ...
    <div v-if="hasUnsavedBuild && currentView === 'list'" class="unsaved-build-indicator">
      <div class="unsaved-content">
        <p>
          <strong>Unsaved Build:</strong> {{ currentBuild.name || 'Unnamed' }}
          <span v-if="currentBuild.warframe_uniqueName">
            with {{ getWarframeName(currentBuild.warframe_uniqueName) }}
          </span>
        </p>
        <div class="unsaved-actions">
          <button @click="continueEditing" class="btn-continue" :disabled="loading">
            Continue Editing
          </button>
          <button @click="saveAsNew" class="btn-save" :disabled="loading">
            Save as New
          </button>
          <button @click="discardBuild" class="btn-discard" :disabled="loading">
            Discard
          </button>
        </div>
      </div>
...
const getWarframeName = (uniqueName: string) => {
  const warframe = warframes.value.find(w => w.uniqueName === uniqueName)
  return warframe?.name || uniqueName.split('/').pop()?.replace('BaseSuit', '') || 'Unknown'
}
...
const saveAsNew = async () => {
  try {
    ...
    alert('Failed to save build: ' + (err instanceof Error ? err.message : 'Unknown error'))
  }
}
...
const handleBuildSubmit = async () => {
  ...
  try {
    if (editingBuild.value?.id) {
      ...
      // Create new build
      await createBuild(formData.value)
