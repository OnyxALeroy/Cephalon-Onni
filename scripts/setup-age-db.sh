...
if docker ps &> /dev/null; then
    ...
    if docker ps --format "table {{.Names}}" | grep -q "cephalon-onni-backend"; then
        ...
        # Run script in backend container
        echo "Running HTML parsers script in backend container..."
        if docker exec -w /app -e PYTHONPATH=/app cephalon-onni-backend python app/html_parsers.py; then
        ...
        echo "Graph Status:"
        node_count=$(docker exec cephalon-onni-postgres psql -U user -d cephalon_db -t -c "
            LOAD 'age';
            SET search_path = ag_catalog, public;
